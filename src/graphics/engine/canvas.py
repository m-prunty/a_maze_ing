...
class Canvas:
    def __init__(self, siz: Vec2, pos: Vec2):
        ...
        self.img_mem, self.bpp, self.siz_line, _ = \
            Mlx_context._mlx.mlx_get_data_addr(self.img)

    
    
    def add_image(self, img_ptr, place: Vec2):
        src_data, src_bpp, src_size_line, _ = \
        Mlx_context._mlx.mlx_get_data_addr(Textures(img_ptr))
        src_data = src_data.cast('B')
    
        bytes_pp = src_bpp // 8
        # siz = Textures.get_siz(img_ptr)
        siz = Vec2()
        ...
        for y in range(siz.y):
            ...
            self.img_mem[dst_start:dst_end] = \
                src_data[src_start:src_end].tobytes()
